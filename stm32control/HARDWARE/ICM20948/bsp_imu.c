...
#include <stdio.h>
//定义imu数据
IMU_DATA_t axis_9Val = { 0 };                   //9轴数值
ATTITUDE_DATA_t AttitudeVal = { 0 };            //欧拉角
...
//获得6轴的数据原始值
	...
	//获得加速度3轴原始值
	accel[0]=data->accel.x = (short)(tmpbuf[0]<<8 | tmpbuf[1]);
	accel[1]=data->accel.y = (short)(tmpbuf[2]<<8 | tmpbuf[3]);
	accel[2]=data->accel.z = (short)(tmpbuf[4]<<8 | tmpbuf[5]);
	...
	//获得角速度3轴原始值
	gyro[0]=data->gyro.x = (short)(tmpbuf[6]<<8 | tmpbuf[7]);
	gyro[1]=data->gyro.y = (short)(tmpbuf[8]<<8 | tmpbuf[9]);
	gyro[2]=data->gyro.z = (short)(tmpbuf[10]<<8 | tmpbuf[11]);
...
//标定零点的方法
static void setZeroPoint(IMU_DATA_t* point,ATTITUDE_DATA_t* attitude) 
{
	point->accel.z -= 9.8f;//Z轴加速度默认为9.8
...
static void attitudeUpdate(IMU_DATA_t imudata, ATTITUDE_DATA_t *attitude) // 显示互补滤波（陀螺仪 + 加速度计）
{
	...
	float halftime = 0.010f*0.5f; // 10ms的采样周期
	...
	attitude -> yaw   -= ZeroAttitude.yaw;
		// 角度范围限制在-180°~180°
		if (attitude -> yaw > 3.1416f) attitude -> yaw -= 6.2832f;
}
