...
class WebDisplay(BaseDisplay):
    ...
    async def update_text(self, text: str):
        """更新文本显示"""
        await self._broadcast_message({"type": "text_update", "data": {"text": text}})
    ...
    async def update_emotion(self, emotion: str):
        ...
        await self._broadcast_message({"type": "emotion_update", "data": {"emotion": normalized_emotion}})

    async def update_status(self, status: str, connected: bool = False):
        """更新状态显示"""
        await self._broadcast_message({"type": "status_update", "data": {"status": status, "connected": connected}})

    async def update_button_status(self, text: str):
        """更新按钮状态"""
        await self._broadcast_message({"type": "button_status_update", "data": {"text": text}})
